...
export const RentFFView: React.FC<RentFFViewProps> = ({
  accounts,
  categories,
  isAdmin,
  onSelectAccount,
    ...
    <div className="animate-fade-in pb-24 text-left">
          ...
          return (
            <button
              ...
              className={`shrink-0 px-3.5 py-1.5 rounded-full text-xs font-bold transition-all flex items-center gap-1 cursor-pointer whitespace-nowrap ${
                isSelected
                  ? 'bg-[#FF9EBB] text-white shadow-sm ring-2 ring-[#FFD1E0]'
                  : 'bg-white text-[#8A6F65] border border-[#FFE4EC] hover:bg-[#FFF0F5]'
              }`}
            >
      ...
      <div className="px-5 my-3 flex items-center justify-between">
        ...
        {isAdmin && onAddNewAccount && (
          ...
          >
            <i className="fa-solid fa-plus text-[10px]"></i> เพิ่มรหัส
          </button>
        )}
      </div>
      ...
      <div className="px-4">
        {displayAccounts.length === 0 ? (
          <div className="text-center text-[#8A6F65] text-xs py-12 bg-white rounded-[24px] border border-dashed border-[#FFE4EC] my-4 shadow-xs">
            <i className="fa-regular fa-folder-open text-2xl text-[#FF9EBB] mb-2 block"></i>
            ยังไม่มีรหัสในหมวดหมู่นี้ หรือสถานะนี้
          </div>
        ) : (
            ...
            {displayAccounts.map((acc) => {
              ...
              return (
                <div
                  ...
                  className="bg-white border-[3px] border-[#FFF9EB] rounded-[24px] shadow-[0_6px_15px_rgba(255,158,187,0.15)] ring-2 ring-[#FFE4EC] flex flex-col group cursor-pointer transition-all duration-300 hover:-translate-y-1 overflow-hidden relative"
                >
                  {/* Image area */}
                  <div className="relative w-full aspect-square bg-[#FFF0F5] overflow-hidden rounded-t-[20px]">
                    <img
                      src={acc.img}
                      alt={acc.title}
                      ...
                      onError={(e) => {
                        (e.currentTarget as HTMLImageElement).src =
                          'https://placehold.co/400x400/FFF0F5/FF9EBB?text=Mednun+FF';
                      }}
                    />
                  ...
                  <div className="p-2.5 flex flex-col flex-grow justify-between bg-white rounded-b-[20px]">
                    <div>
                      ...
                      <p className="text-[10px] text-[#FF9EBB] font-bold truncate">
                        {acc.rates?.map((r) => r.label).join(' | ') || 'ทักแชทเพื่อเช่า'}
                      </p>
                    </div>

                    <div className="mt-2 w-full bg-[#FFF0F5] hover:bg-[#FFE4EC] text-[#D9779B] py-1.5 rounded-xl text-[10px] font-bold flex items-center justify-center gap-1 transition-colors">
                      <span>เช่ารหัส</span>
