...
interface RentBottomSheetProps {
...
export const RentBottomSheet: React.FC<RentBottomSheetProps> = ({
  item,
  gameType,
  onClose,
  onGoToContact,
  ...
  const copyText = selectedRate
    ? `สนใจเช่ารหัส${gameType === 'ROV' ? ' Rov' : ''}: ${item.title}\nเรทที่ต้องการ: ${selectedRate}`
    : `สนใจเช่ารหัส${gameType === 'ROV' ? ' Rov' : ''}: ${item.title}`;
  ...
  const isAvailable = item.status === 'ว่าง';
        ...
        <div className="overflow-y-auto hide-scrollbar px-5 pb-8 flex-1">
          <div className="bg-white rounded-[28px] p-4 shadow-sm border border-[#FFE4EC] flex flex-col">
              ...
              <img
                ...
                onError={(e) => {
                  (e.currentTarget as HTMLImageElement).src =
                    'https://placehold.co/400x400/FFF0F5/FF9EBB?text=Mednun+Game';
                }}
              />
              <div
            ...
            <div className="w-full text-left px-1">
              <h3 className="font-extrabold text-[#6B4C42] text-[16px] mb-3 tracking-wide leading-tight">
                {item.title}
              </h3>
                  ...
                  return (
                    <button
                      ...
                      className={`text-[11px] font-bold px-3.5 py-1.5 rounded-full transition-all border cursor-pointer active:scale-95 ${
                        isChosen
                          ? 'bg-[#FF9EBB] text-white border-[#FF9EBB] shadow-sm scale-102 ring-2 ring-[#FFD1E0]'
              ...
              <div className="border-t-[1.5px] border-dashed border-[#FFD1E0] pt-3 mb-3">
                <p className="text-[10px] text-[#8A6F65] font-medium text-center leading-relaxed">
                  แคปหน้าจอนี้แล้วส่งให้แอดมิน หรือกดที่เรทราคาเพื่อคัดลอกข้อความ
                </p>
              </div>
              ...
              >
                ...
                <span>ติดต่อแอดมินเพื่อเช่ารหัส</span>
              </button>
            </div>
          </div>
        </div>
